...
public class LinkFunction implements Function, ServerExtension {
  ...
  protected void createJiraIssue(Context context) {
    RemoteIssue issue;
    try {
      issue = jiraIssueCreator.createIssue(context.issue(), context.projectSettings(), formatComments(context.issue().comments()));
  ...
  private String formatComments(List<IssueComment> list) {
	  StringBuffer comments = new StringBuffer();
	  SimpleDateFormat format = new SimpleDateFormat("dd/MM/yy HH:mm");
	  for (IssueComment comment : list){		
		  comments.append("_");
		  comments.append(comment.userLogin());
		  comments.append("_");
		  comments.append(": ");
		  comments.append(comment.markdownText());
		  comments.append(" (");
		  comments.append(format.format(comment.createdAt()));
		  comments.append(") ");
		  comments.append("\n");
	  }
